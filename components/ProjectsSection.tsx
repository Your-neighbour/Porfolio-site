  ...
  const projects: Project[] = [
    {
      ...
      fullDescription:
        'A comprehensive e-commerce platform built from the ground up with modern web technologies. Features include user authentication, product management, shopping cart functionality, and secure payment processing through Stripe.',
      media: [
        {
          type: 'image',
      ...
      challenges:
        'Implementing real-time inventory updates and ensuring secure payment processing while maintaining a smooth user experience.',
      outcome:
        'Successfully launched platform serving 10,000+ users with 99.9% uptime and processing $500K+ in transactions.',
    },
    {
      ...
      fullDescription:
        'A powerful task management solution designed for teams to collaborate effectively. Built with real-time synchronization using Firebase, allowing team members to see updates instantly.',
      media: [
        {
          type: 'image',
      ...
      challenges:
        'Managing complex state across multiple users while maintaining real-time synchronization without conflicts.',
      outcome:
        'Adopted by 50+ teams, improving task completion rates by 35% and reducing project delays by 40%.',
    },
    {
      ...
      fullDescription:
        'An innovative content generation tool leveraging OpenAI API to help users create high-quality content quickly. Features advanced prompt engineering and content refinement capabilities.',
      media: [
        {
          type: 'image',
      ...
      challenges:
        'Optimizing API calls to balance response time and cost while maintaining high-quality output.',
      outcome:
        'Generated 100,000+ pieces of content, saving users an average of 5 hours per week in content creation.',
    },
  ]
  return (
...
function ProjectCard({
  ...
  return (
    <div
      className="group rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden hover:shadow-lg hover:border-primary/50 transition-all cursor-pointer"
      onClick={onClick}
    >
      <div className="aspect-video w-full overflow-hidden relative">
        {currentMedia.type === 'image' ? (
          ...
          <video
            ...
            aria-label={currentMedia.alt || 'Project video'}
          />
        )}
          ...
          <>
            ...
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex space-x-1">
              {project.media.map((_, idx) => (
                <span
                  key={idx}
                  className={`block h-1.5 w-1.5 rounded-full ${currentMediaIndex === idx ? 'bg-primary' : 'bg-background/80'}`}
                />
              ))}
            </div>
          </>
            ...
            <div className="aspect-video w-full overflow-hidden relative rounded-lg mb-6">
              ...
              {project.media.length > 1 && (
                <>
                  <button
                    onClick={prevMedia}
                    className="absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-background/90 hover:bg-background transition-colors"
                    aria-label="Previous media"
                  >
                    <ChevronLeft size={24} />
                  </button>
                  <button
                    onClick={nextMedia}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-background/90 hover:bg-background transition-colors"
                    aria-label="Next media"
                  >
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex space-x-2">
                    {project.media.map((_, idx) => (
                      <span
                        key={idx}
                        className={`block h-2 w-2 rounded-full ${currentMediaIndex === idx ? 'bg-primary' : 'bg-background/80'}`}
                      />
                    ))}
                  </div>
                </>
